...
import { LayoutDashboard, Users, FileText, Send, LogOut, Menu, X } from 'lucide-react';
  ...
  const NavContent = () => (
    <>
      <div className="tw-flex tw-h-16 tw-shrink-0 tw-items-center tw-px-6">
        <span className="tw-text-xl tw-font-bold tw-text-white">Admin</span>
      </div>
      <nav className="tw-flex tw-flex-1 tw-flex-col tw-px-4 tw-py-4">
        <ul className="tw-flex tw-flex-1 tw-flex-col tw-gap-1">
          {navigation.map((item) => {
    ...
    <>
      ...
      {mobileOpen && (
        <div className="tw-fixed tw-inset-0 tw-z-40 lg:tw-hidden">
          ...
          <div className="tw-fixed tw-inset-y-0 tw-left-0 tw-w-64 tw-bg-gray-900">
            <NavContent />
          </div>
        </div>
      )}
